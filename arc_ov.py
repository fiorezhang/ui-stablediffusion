...
def generateImage(xpu, ov_pipe, prompt='horse', negative='', seed=0, steps=20, callback=None):
    ...
    if BETA_MODE:
        result = ov_pipe(prompt, negative_prompt=negative, num_inference_steps=int(steps), seed=int(seed), gif=False, callback=callback)
    else:
        result = ov_pipe(prompt, negative_prompt=negative, num_inference_steps=int(steps), seed=int(seed), gif=False)

    str_image = validateTitle(str(prompt))[:100] + '_seed_' + str(seed) + '_step_' + str(steps)
